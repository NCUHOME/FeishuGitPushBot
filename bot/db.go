...
type MessageRecord struct {
	...
	GithubID        string    `bun:",unique,notnull"` // workflow_run_id or branch_ref or sha
	FeishuMessageID string    `bun:",notnull"`
	ChatID          string    `bun:",notnull"`
	RepoName        string    `bun:",notnull"`
	Ref             string    `bun:""`
	EventType       string    `bun:",notnull"`
	Content         string    `bun:"type:text"` // Stores card detail JSON
	RawPayload      string    `bun:"type:text"` // Original webhook payload
...
type ImageCache struct {
	...
	ExpireAt time.Time `bun:",nullzero"` // Optional: for cleanup
}
